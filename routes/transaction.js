...
router.get('/:id',accessControl(['super_admin', 'customer']), paymentController.fetchOne);
router.param('id', paymentController.validatepayment);
router.post('/', accessPermission(['customer', 'merchant']), paymentController.create);
router.put('/:id', accessControl(['admin', 'super_admin','helper']), paymentController.update);
